...
import { ResponsePolicyTab } from './ResponsePolicyTab'
  ...
  return (
    <>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)' }}>Response Policy</h2>
        <p style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '3px' }}>
          Controls output format, tone, token limits, and streaming defaults per assistant.
        </p>
      </div>
      ...
      {!selectedAssistant ? (
        ...
        >
          Select an assistant above to view and edit its response policy.
        </div>
      ) : (
        <div
          style={{
        ...
        >
          ...
          >
            <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text)' }}>
              💬 Response Policy — {selectedAssistant}
            </div>
          </div>
          <div style={{ padding: '20px' }}>
            <ResponsePolicyTab assistantCode={selectedAssistant} />
          </div>
        </div>
      )}
    </>
