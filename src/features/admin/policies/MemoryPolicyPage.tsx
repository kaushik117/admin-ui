...
import { MemoryPolicyTab } from './MemoryPolicyTab'
  ...
  return (
    <>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)' }}>Memory Policy</h2>
        <p style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '3px' }}>
          One policy per assistant. Controls conversation memory storage, window size, and TTL.
        </p>
      </div>
      ...
      {!selectedAssistant ? (
        ...
        >
          Select an assistant above to view and edit its memory policy.
        </div>
      ) : (
        <div
          style={{
        ...
        >
          ...
          >
            <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text)' }}>
              🧠 Memory Policy — {selectedAssistant}
            </div>
          </div>
          <div style={{ padding: '20px' }}>
            <MemoryPolicyTab assistantCode={selectedAssistant} />
          </div>
        </div>
      )}
    </>
